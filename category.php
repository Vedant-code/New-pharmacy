...
if (isset($_REQUEST['form_action'])) {
    ...
    {
        if (preg_match('/^([0-9]{3})|([0-9]{2})|([0-9]{1})$/', $age)) {
            return true;
        } else {
            return false;
        }
    ...
    // Validation :
    if (validate_age($lower_age) == false) {
        $error_msg .= "Invalid lower age.<br/>";
    }

    if (validate_age($upper_age) == false) {
        $error_msg .= "Invalid upper age.<br/>";
    }

    if (validate_category($category_name) == false) {
        $error_msg .= "No such category exist.<br/>";
    }
